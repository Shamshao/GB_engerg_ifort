...
contains
    ...
    function GB5DOF(P, Q, AlCuParameter, eRGB) result(en)
        ...
        real(8), intent(in) :: AlCuParameter     ! Material parameter
        real(8), intent(in), optional :: eRGB   ! Energy scaling parameter

        ! Output argument
        real(8) :: en

        ! Local variables
        real(8),dimension(3) :: geom100, geom110, geom111
        real(8), allocatable :: parvec(:)

        ! Calculate distances for different crystal planes
        geom100 = distances_to_set(P, Q, '100')
        geom110 = distances_to_set(P, Q, '110')
        geom111 = distances_to_set(P, Q, '111')

        ! Check if eRGB is provided and calculate parameters accordingly
        if (present(eRGB)) then
            parvec = makeparvec(AlCuParameter,eRGB)  ! Option 2: use AlCuParameter
        else
            parvec = makeparvec(AlCuParameter)  ! Option 1: use AlCuParameter and eRGB
        end if

        ! Calculate the energy using the weighted mean energy
    ...
    ! Function to compute the weighted mean energy
    real(8) function weightedmeanenergy(geom100, geom110, geom111, parvec)
        implicit none
        real(8), intent(in) :: geom100, geom110, geom111
        real(8), intent(in) :: parvec(:)
        real(8) :: energy

        ! Implement the weighted mean energy calculation
        ! Placeholder implementation for demonstration
...
contains
    ...
    function distances_to_set(P, Q, whichaxes, dismax) result(geom)
        ...
        real(8), dimension(3, :), allocatable :: axes, dirs
